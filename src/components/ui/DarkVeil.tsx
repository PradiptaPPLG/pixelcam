...
export default function DarkVeil({
  ...
  // Keep propsRef in sync with updates
  useEffect(() => {
    propsRef.current = {
      hueShift,
      noiseIntensity,
  ...
  useEffect(() => {
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        ...
        uLightMode: { value: propsRef.current.isDarkMode ? 0.0 : 1.0 },
      },
    });

    const mesh = new Mesh(gl, { geometry, program });

    // Handle resizing using a ResizeObserver on the parent node
    const resize = () => {
    ...
    const loop = () => {
      ...
      program.uniforms.uTime.value = ((performance.now() - start) / 1000) * current.speed;
      program.uniforms.uHueShift.value = current.hueShift;
      program.uniforms.uNoise.value = current.noiseIntensity;
      program.uniforms.uScan.value = current.scanlineIntensity;
      program.uniforms.uScanFreq.value = current.scanlineFrequency;
      program.uniforms.uWarp.value = current.warpAmount;
      program.uniforms.uLightMode.value = current.isDarkMode ? 0.0 : 1.0;
